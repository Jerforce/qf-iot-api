...
public class JwtUtil {
    private static final String salt = "wangbiao";
    ...
    public static Tenant verifyToken(String token){
            JWTVerifier jwtVerifier = JWT.require(Algorithm.HMAC256(salt)).build();
            DecodedJWT verify = jwtVerifier.verify(token);
        return  JSONUtil.toBean(verify.getClaim("user").asString(),Tenant.class);
    }
}
